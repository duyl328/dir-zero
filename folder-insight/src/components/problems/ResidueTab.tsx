...
// Inline form for adding a custom rule
  ...
  return (
    <div className="bg-surface-container-lowest rounded-xl border border-primary/30 p-4 space-y-3">
      ...
      <div className="flex gap-2 justify-end">
        <button
          onClick={onCancel}
          className="px-4 py-1.5 rounded-lg text-xs font-semibold text-on-surface-variant hover:bg-surface-container-high transition-colors"
        >
          取消
        </button>
        <button
          onClick={handleSubmit}
          className="px-4 py-1.5 rounded-lg text-xs font-bold bg-primary text-on-primary hover:opacity-90 transition-opacity"
        >
          添加
        </button>
      </div>
...
export default function ResidueTab({ groups: initialGroups, onRefresh, onAddRule }: Props) {
  ...
  // Auto-select newly added groups (those not yet in selected set)
  useEffect(() => {
    setSelected((prev) => {
  ...
  const activeGroups = groups.filter((g) => selected.has(g.ruleId));
  ...
  if (groups.length === 0 && !showAddForm) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-20 text-center">
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold text-primary bg-primary/10 hover:bg-primary/15 transition-colors"
        >
      ...
      {groups.length > 1 && (
        <div className="flex items-center gap-3 mb-3">
          ...
          <span className="text-[10px] text-on-surface-variant/50">
            已选 {selected.size} / {groups.length} 类规则
          </span>
        </div>
      )}
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3 p-4">
                {/* Selection indicator — large and obvious */}
                <button
                  onClick={() => toggleSelect(group.ruleId)}
                ...
                {/* Info — clicking also toggles selection */}
                <div className="flex-1 min-w-0 cursor-pointer" onClick={() => toggleSelect(group.ruleId)}>
                  <p className={[
              ...
              {isExpanded && (
                <div className="border-t border-outline-variant/10 max-h-52 overflow-y-auto">
                  {group.files.map((f: FileEntry) => (
                    <button
                      key={f.path}
                      onClick={() => revealItemInDir(f.path).catch(() => {})}
                      title={`在文件管理器中打开\n${f.path}`}
                      className="w-full flex items-center gap-3 px-4 py-2 border-b border-outline-variant/5 last:border-0 hover:bg-surface-container-high transition-colors text-left"
                    >
                      ...
                      <span className="material-symbols-outlined text-[13px] text-on-surface-variant/30 shrink-0">open_in_new</span>
                    </button>
                  ))}
                </div>
              )}
