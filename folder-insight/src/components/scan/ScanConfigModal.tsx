...
export default function ScanConfigModal() {
  ...
  async function handleAddFolder() {
    try {
      const selected = await open({ directory: true, multiple: true, title: t.scanConfig.selectFolderDialog });
      if (selected) {
      ...
      <div className="bg-surface-container-lowest rounded-2xl shadow-2xl border border-outline-variant/10 w-full max-w-2xl mx-4 overflow-hidden">
        ...
        <div className="px-8 pt-8 pb-6 border-b border-outline-variant/10">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="font-headline text-xl font-extrabold text-on-surface">{t.scanConfig.title}</h2>
              <p className="text-sm text-on-surface-variant mt-1">{t.scanConfig.subtitle}</p>
            </div>
            <button onClick={handleCancel} className="p-2 text-on-surface-variant hover:bg-surface-container-high rounded-lg transition-colors">
        ...
        <div className="px-8 py-6 space-y-6 max-h-[60vh] overflow-y-auto">
          <section>
            <h3 className="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-3">{t.scanConfig.scanTargets}</h3>
            <div className="space-y-2">
              {roots.map((r) => (
                <div key={r} className="flex items-center justify-between px-4 py-2.5 bg-surface-container-low rounded-lg group">
                  ...
                  <button onClick={() => handleRemoveRoot(r)} className="text-on-surface-variant hover:text-error opacity-0 group-hover:opacity-100 transition-all ml-2 shrink-0">
              ...
              <button onClick={handleAddFolder}
                ...
                {t.scanConfig.addFolder}
              </button>
            </div>
          </section>

          <section>
            <h3 className="text-xs font-bold uppercase tracking-widest text-on-surface-variant mb-3">{t.scanConfig.excludeRules}</h3>
            <div className="space-y-4">
              {builtinGroups.map((group) => (
                <div key={group.label}>
                  <p className="text-[11px] font-bold text-on-surface-variant/60 uppercase tracking-wider mb-2">{group.label}</p>
                  <div className="flex flex-wrap gap-2">
                    {group.ids.map((id) => {
                      const rule = excludeRules.find((r) => r.id === id);
                      if (!rule) return null;
                      return (
                        <button key={id} onClick={() => toggleExcludeRule(id)}
                          className={["flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all",
                            rule.enabled ? "bg-primary/10 text-primary border border-primary/20" : "bg-surface-container-high text-on-surface-variant border border-transparent hover:border-outline-variant/30"].join(" ")}>
                          {rule.enabled && <span className="material-symbols-outlined text-[14px]">check</span>}
                          {rule.label}
                        </button>
                      );
                    })}
        ...
        <div className="px-8 py-5 border-t border-outline-variant/10 flex items-center justify-between bg-surface-container-low/50">
          <div className="flex items-center gap-2 text-xs text-on-surface-variant">
            ...
            <span>{t.scanConfig.ntfsFastScan}</span>
          </div>
          <div className="flex items-center gap-3">
            <button onClick={handleCancel} className="px-5 py-2 rounded-lg text-sm font-semibold text-on-surface-variant hover:bg-surface-container-high transition-colors">
              {t.scanConfig.cancel}
            </button>
            <button onClick={handleStartScan} disabled={roots.length === 0}
