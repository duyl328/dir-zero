...
import type { SlimFolderEntry, FolderChild } from "../../types";
...
function folderHasMatch(folder: SlimFolderEntry, matchedPaths: Set<string>): boolean {
  for (const child of folder.children) {
    if (folderHasMatch(child, matchedPaths)) return true;
    // Also check if the folder path itself matches (for search results navigating to parent)
    if (matchedPaths.has(child.path)) return true;
  }
  return false;
}

function countFolderMatches(folder: SlimFolderEntry, matchedPaths: Set<string>): number {
  let n = 0;
  for (const child of folder.children) {
    if (matchedPaths.has(child.path)) n++;
    n += countFolderMatches(child, matchedPaths);
  }
  return n;
}

interface TreeNodeProps {
  child: SlimFolderEntry;
  depth: number;
  selected: FolderChild | null;
  currentFolderPath?: string;
  onSelect: (node: FolderChild) => void;
  onContextMenu: (e: React.MouseEvent, child: SlimFolderEntry) => void;
  matchedPaths?: Set<string>;
}

function TreeNode({ child, depth, selected, currentFolderPath, onSelect, onContextMenu, matchedPaths }: TreeNodeProps) {
  const shouldExpandForCurrent = currentFolderPath
    ? isAncestorOrSelf(child.path, currentFolderPath)
    : false;

  const shouldExpandForSelected = selected !== null
    ? isAncestorOrSelf(child.path, selected.path) && selected.path !== child.path
    : false;

  const isSearching = matchedPaths !== undefined;

  const shouldExpandForSearch = isSearching
  ...
  useEffect(() => {
    ...
    else if (isSearching) setExpanded(false);
  ...
  const isCurrentFolder = child.path === currentFolderPath;
  const isAncestorOfSelected = !isSelected && selected !== null
    && isAncestorOrSelf(child.path, selected.path);

  const isFolderWithMatches = isSearching && folderHasMatch(child, matchedPaths!);
  const isDimmed = isSearching && !isFolderWithMatches;

  const hasChildren = child.children.length > 0;
  const matchCount = isSearching ? countFolderMatches(child, matchedPaths!) : 0;

  return (
    <div className={isDimmed ? "opacity-30" : undefined}>
      <button
        onClick={() => {
          onSelect({ kind: "folder", ...child } as import("../../types").FolderChild);
      ...
      {expanded && hasChildren && (
        <div>
          {child.children.slice(0, 50).map((c) => (
            <TreeNode
              key={c.path}
              child={c}
              depth={depth + 1}
...
export default function DirectoryTree({ root, selected, onSelect, currentFolderPath, onNavigateToRoot, matchedPaths }: Props) {
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; child: SlimFolderEntry } | null>(null);
  const isAtRoot = !currentFolderPath || currentFolderPath === root.path;

  function handleContextMenu(e: React.MouseEvent, child: SlimFolderEntry) {
    e.preventDefault();
